...
impl Analyzer<'_> {
    ...
    /// there.
    fn record_this_usage(&mut self, id: Id, usage: impl FnOnce() -> ThisUsage) {
...
impl Visit for Analyzer<'_> {
    ...
    fn visit_export_default_decl(&mut self, n: &ExportDefaultDecl) {
        ...
        if let DefaultDecl::Fn(f) = &n.decl {
            ...
            self.record_this_usage(id, || function_this_usage(&f.function, unresolved_mark));
        }
        n.visit_children_with(self);
    }
    ...
    fn visit_export_default_expr(&mut self, n: &ExportDefaultExpr) {
        ...
        let unresolved_mark = self.unresolved_mark;
        self.record_this_usage(default_id, || value_this_usage(&n.expr, unresolved_mark));
        n.visit_children_with(self);
    }
    ...
    fn visit_fn_decl(&mut self, node: &FnDecl) {
        let unresolved_mark = self.unresolved_mark;
        self.record_this_usage(node.ident.to_id(), || {
    ...
    fn visit_var_declarator(&mut self, node: &VarDeclarator) {
        ...
        if let Some(init) = &node.init {
            ...
            if let Pat::Ident(binding) = &node.name {
                self.record_this_usage(binding.to_id(), || value_this_usage(init, unresolved_mark));
                ...
                for id in find_pat_ids::<_, Id>(&node.name) {
                    self.record_this_usage(id, || ThisUsage::MaybeUsingThis);
                }
            }
        }
        node.visit_children_with(self);
    ...
    fn visit_assign_expr(&mut self, node: &AssignExpr) {
        ...
        {
            let unresolved_mark = self.unresolved_mark;
            self.record_this_usage(i.to_id(), || value_this_usage(&node.right, unresolved_mark));
        } else {
            for id in find_pat_ids::<_, Id>(&node.left) {
                self.record_this_usage(id, || ThisUsage::MaybeUsingThis);
            }
