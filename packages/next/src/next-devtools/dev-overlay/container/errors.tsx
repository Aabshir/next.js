...
export function Errors({
  ...
  const [previousInstantErrorCount, setPreviousInstantErrorCount] = useState(
    instantErrors.length
  )
  if (previousInstantErrorCount !== instantErrors.length) {
    setPreviousInstantErrorCount(instantErrors.length)
    // The advisory follows runtime insights; keep it selected as they change.
    if (advisory && activeIndices.instant === previousInstantErrorCount) {
      setActiveIndices({ ...activeIndices, instant: instantErrors.length })
    } else if (activeIndices.instant >= instantErrors.length) {
      setActiveIndices({
        ...activeIndices,
        instant: Math.max(0, instantErrors.length - 1),
      })
    }
  }
